...
export class Generador {
    ...
    sgt(rd, rs1, rs2) {
        this.instrucciones.push(new Instruction('sgt', rd, rs1, rs2))
    }
    sle(rd, rs1, rs2) {
        this.instrucciones.push(new Instruction('sle', rd, rs1, rs2))
    }
    sge(rd, rs1, rs2) {
        this.instrucciones.push(new Instruction('sge', rd, rs1, rs2))
    }
    and(rd, rs1, rs2) {
    ...
    pushConstant(object) {
        ...
        switch (object.type) {
            ...
            case 'float':
                const floatArray = stringTo32BitsArray(object.valor);

                this.comment(`Pushing float ${object.valor}`);
                this.push(r.HP);

                floatArray.forEach((charCode) => {
                    this.li(r.T0, charCode);
                    this.sb(r.T0, r.HP);
                    this.addi(r.HP, r.HP, 1);
                });
